 ...
 * Credit to LibGDX and ThinMatrix.
 * <p>
 * Thanks to Riven on JavaGaming.org for the basis of sin/cos/floor/ceil.
 * @author Nathan Sweet */
public class Maths {
	
	// Credit to LibGDX for everything in the following section
	...
	//Credit to ThinMatrix for everything in the following section
	...
	public static Colour mix( Colour start, Colour end, float mix ) { return mix( start, end, mix, new Colour() ); }
	public static Colour mix( Colour start, Colour end, float mix, Colour dest ) {
		dest.set( mix( start.getVector(), end.getVector(), mix ), mix( start.getA(), end.getA(), mix ) );
		return dest;
	}
	
	public static Vector3f mix( Vector3f start, Vector3f end, float mix ) { return mix( start, end, mix, new Vector3f() ); }
	public static Vector3f mix( Vector3f start, Vector3f end, float mix, Vector3f dest ) {
		dest.set( mix( start.x, end.x, mix ), mix( start.y, end.y, mix ), mix( start.z, end.z, mix ) );
		return dest;
	}
	//FIXME: doesn't check for non-normalized mix
	public static float mix( float start, float end, float mix ) {
